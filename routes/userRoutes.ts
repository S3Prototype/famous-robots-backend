...
router.post("/login", authenticateToken, (req, res, next) => {
  ...
  userModel.findOne({ email: req.body.email }, async (err, userData) => {
    if (err) {
      return res.status(502).json({
        success: false,
        message: `Failed to login ${req.body.email}. Possible database rror. Please try again.`,
      });
    }
...
router.post("/logout", async (req, res) => {
  try {
    tokenContainer = tokenContainer.filter(
      (token) =>
    ...
    return res.send({
      info: `Failed to logout user ${req.body.email}, ${err}`,
    });
  }
